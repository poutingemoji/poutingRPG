...
module.exports = class AnimeCommand extends Command {
  ...
  async run(msg, { anime }) {
    try {
      ...
      if (animeRequest["data"][0] === undefined)
        return sentMessage.edit(
          `${emoji(msg, "err")} Could not find info on ${anime}`
        );
    } catch (err) {
      console.error(err);
    }
    ...
    sentMessage
      .edit(
        `${msg.author}, I have found about ${animeRequest["data"].length} results, please pick the one you meant.\n${possibleMatches}`
      )
      .then((msgSent) => {
        msgSent.channel
          ...
          .then((res) => {
            ...
            msgSent.delete();
            const title = animeInfo["titles"]["ja_jp"]
              ? `${animeInfo["titles"]["ja_jp"]} - ${animeInfo["canonicalTitle"]}`
              : animeInfo["canonicalTitle"];
            const messageEmbed = new MessageEmbed()
              .setColor("#ed7220")
              .setTitle(title)
              .setURL(
                `https://kitsu.io/anime/${animeRequest["data"][chosenAnimeIndex]["id"]}`
              )
