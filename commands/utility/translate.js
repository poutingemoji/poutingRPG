...
const { titleCase, codeBlock } = require("../../utils/helpers/strHelper");
...
module.exports = class TranslateCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      examples: [
        `${client.commandPrefix}translate [language/ISO 639-1] [text]`,
      ],
      clientPermissions: [],
      userPermissions: [],
      args: [
  ...
  run(msg, { language, content }) {
    ...
    if (!translate.languages.getCode(language)) {
      return msg.say(
        `${emoji(msg, "err")} ${msg.author}, the language, **${titleCase(
          language
    ...
    translate(content, opts)
      .then((res) => {
        const messageEmbed = new MessageEmbed()
          ...
          .addField(
            translate.languages[res.from.language.iso],
            codeBlock(content)
          )
          .setTimestamp()
