...
module.exports = class MangaCommand extends Command {
  ...
  async run(msg, { manga }) {
    try {
      ...
      if (mangaRequest["data"][0] === undefined)
        return sentMessage.edit(
          `${emoji(msg, "err")} Could not find info on ${manga}`
        );
    } catch (err) {
      console.error(err);
    }
    ...
    sentMessage
      .edit(
        `${msg.author}, I have found about ${mangaRequest["data"].length} results, please pick the one you meant.\n${possibleMatches}`
      )
      .then((msgSent) => {
        msgSent.channel
          .awaitMessages(filter, { max: 1, time: 12000 })
          .then((res) => {
            const chosenMangaIndex = res.first().content - 1;
            mangaInfo = mangaRequest["data"][chosenMangaIndex]["attributes"];
            sentMessage.delete();
            //console.log(mangaInfo)
            const title = mangaInfo["titles"]["ja_jp"]
              ? `${mangaInfo["titles"]["ja_jp"]} - ${mangaInfo["canonicalTitle"]}`
              : mangaInfo["canonicalTitle"];
            const messageEmbed = new MessageEmbed()
              .setColor("#ed7220")
              .setTitle(title)
              .setURL(
                `https://kitsu.io/anime/${mangaRequest["data"][chosenMangaIndex]["id"]}`
              )
            ...
            if (mangaInfo["nextRelease"]) {
              messageEmbed.setFooter(
                `Next Release: ${dateFormat(
                  mangaInfo["nextRelease"],
          ...
          .catch((err) => {
            ...
            msg.say(
              `${emoji(msg, "err")} ${
                msg.author
              }, you didn't answer in time. This search is cancelled.`
            );
          });
      });
  }
