...
class Game extends BaseHelper {
      ...
      if (updatedPlayer.health <= 0) {
        ...
        eventMsg.push(
          this.setImportantMessage(
            `${updatedPlayer.name}${
              updatedPlayer.titles.current !== "None"
                ? ` the ${updatedPlayer.titles.current}`
                : ""
            } died${expLoss === 0 ? "" : ` and lost ${expLoss} exp`}${
              goldLoss === 0 ? "" : ` and lost ${goldLoss} gold`
            }! Game over man... Game over.`
          )
        );
        eventLog.push(
          `You died${expLoss === 0 ? "" : ` and lost ${expLoss} exp`}${
            goldLoss === 0 ? "" : ` and lost ${goldLoss} gold`
          }. Game over man... Game over.`
        );
        let bountyEventLog;
        ...
        if (breakChance < 15) {
          ...
          switch (randomEquip) {
            case 0:
              if (
                updatedPlayer.equipment.helmet.name !==
                enumHelper.equipment.empty.helmet.name
              ) {
                eventMsg.push(
                  this.setImportantMessage(
                    `${updatedPlayer.name}${
                      updatedPlayer.titles.current !== "None"
                        ? ` the ${updatedPlayer.titles.current}`
                        : ""
                    }'s ${updatedPlayer.equipment.helmet.name} just broke!`
                  )
                );
                eventLog.push(
                  `Your ${updatedPlayer.equipment.helmet.name} just broke!`
                );
            ...
            case 1:
              if (
                updatedPlayer.equipment.armor.name !==
                  ...
                  this.setImportantMessage(
                    `${updatedPlayer.name}${
                      updatedPlayer.titles.current !== "None"
                        ? ` the ${updatedPlayer.titles.current}`
                        : ""
                    }'s ${updatedPlayer.equipment.armor.name} just broke!`
                  )
                );
                eventLog.push(
                  `Your ${updatedPlayer.equipment.armor.name} just broke!`
                );
                this.setPlayerEquipment(
                  updatedPlayer,
                  ...
                  this.setImportantMessage(
                    `${updatedPlayer.name}${
                      updatedPlayer.titles.current !== "None"
                        ? ` the ${updatedPlayer.titles.current}`
                        : ""
                    }'s ${updatedPlayer.equipment.weapon.name} just broke!`
                  )
                );
                eventLog.push(
                  `Your ${updatedPlayer.equipment.weapon.name} just broke!`
                );
                this.setPlayerEquipment(
                  updatedPlayer,
        ...
        if (!updatedAttacker.discordId) {
          ...
          if (updatedPlayer.currentBounty > 0) {
            ...
            bountyEventLog = `Claimed ${bountyGain} gold for ${
              updatedPlayer.name
            }${
              updatedPlayer.titles.current !== "None"
                ? ` the ${updatedPlayer.titles.current}`
            ...
            eventMsg.push(
              this.setImportantMessage(
                `${updatedAttacker.name}${
                  updatedAttacker.titles.current !== "None"
                    ? ` the ${updatedAttacker.titles.current}`
                    : ""
                } just claimed ${bountyGain} gold as a reward for killing ${
                  updatedPlayer.name
                }${
                  updatedPlayer.titles.current !== "None"
                    ? ` the ${updatedPlayer.titles.current}`
            ...
            otherPlayerPmMsg.push(
              `${updatedAttacker.name}${
                updatedAttacker.titles.current !== "None"
                  ? ` the ${updatedAttacker.titles.current}`
                  : ""
              } just claimed ${bountyGain} gold as a reward for killing you!`
            );
        ...
        eventMsg.push(
          `\`\`\`css\n${updatedPlayer.name}${
            updatedPlayer.titles.current !== "None"
              ? ` the ${updatedPlayer.titles.current}`
              : ""
          } is now level ${updatedPlayer.level}!\`\`\``
        );
          ...
          eventMsg.push(
            `\`\`\`css\n${updatedPlayer.name}${
              updatedPlayer.titles.current !== "None"
                ? ` the ${updatedPlayer.titles.current}`
                : ""
            } has decided to become a ${updatedPlayer.class}!\`\`\``
          );
  ...
  manageTitles(eventResults, title) {
    ...
    if (titles[title].stat.includes(".")) {
      ...
      if (
        eventResults.updatedPlayer[statSplit[0]][statSplit[1]] >=
          titles[title].requirements &&
        ...
        if (eventResults.pm) {
          eventResults.pm.push(
            `You've just unlocked the ${titles[title].name} title!`
          );
        } else {
          eventResults.pm = [
            `You've just unlocked the ${titles[title].name} title!`,
        ...
        eventResults.updatedPlayer.titles.unlocked.splice(
          eventResults.updatedPlayer.titles.unlocked.indexOf(
            titles[title].name
          ),
          1
        );
        if (eventResults.pm) {
          eventResults.pm.push(
            `You've just lost the ${titles[title].name} title!`
          );
        } else {
          eventResults.pm = [
            `You've just lost the ${titles[title].name} title!`,
    ...
    if (
      eventResults.updatedPlayer[titles[title].stat] >=
        titles[title].requirements &&
      !eventResults.updatedPlayer.titles.unlocked.includes(titles[title].name)
    ) {
      if (eventResults.updatedPlayer.titles.current === "None") {
